...
describe('E2E: Premium Features User Story', () => {
  ...
  describe('Account settings display', () => {
    ...
    test('should show subscription status in account settings', async () => {
      ...
      expect(mockChrome.i18n.getMessage).toHaveBeenCalledWith('subscriptionStatus');
    });
  ...
  describe('Account settings interaction', () => {
    ...
    test('should handle manage subscription button click', async () => {
      ...
      popupContext.manageSubscriptionBtn.addEventListener = jest.fn((event, handler) => {
        if (event === 'click') {
          handleManageSubscription.mockImplementation(handler);
        }
      });

      popupContext.manageSubscriptionBtn.addEventListener('click', handleManageSubscription);
      handleManageSubscription();

      expect(handleManageSubscription).toHaveBeenCalled();
    });
