...
describe("E2E: Paid Plan Features", () => {
    ...
    beforeEach(() => {
        ...
        // Mock AccountService that reads from storedPlan
        mockAccountService = {
            enforceReminderLimit: jest.fn(
                async (userId, currentReminderCount) => {
                    const limit = storedPlan.activeReminderLimit;

                    // -1 means unlimited (paid plan)
                    if (limit === PLAN_LIMITS.PAID_ACTIVE_REMINDER_LIMIT) {
                        return { allowed: true, limit };
                    }

                    // Check against free plan limit
                    if (currentReminderCount >= limit) {
                        return {
                            allowed: false,
            ...
            getUserPlan: jest.fn(async (userId) => {
                return storedPlan.planType;
            }),
            getReminderLimit: jest.fn(async (userId) => {
                return storedPlan.activeReminderLimit;
            }),
        };
    });

    /**
     * Helper: creates N reminders using mock AccountService validation.
     * @param {number} count
    ...
    async function createReminders(count) {
        ...
        for (let i = 0; i < count; i++) {
            const reminder = await ReminderService.createReminder(
                ...
                { storage: mockStorage, account: mockAccountService },
            );
