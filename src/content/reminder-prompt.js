...
(function () {
    ...
    function calculatePresets() {
        ...
        presets.push({
            label: i18n("presetOneHour", "In 1 hour"),
            time: oneHour,
        });
        ...
        presets.push({
            label: i18n("presetTonight", "Tonight at 8 PM"),
            time: tonight,
        });
        ...
        presets.push({
            label: i18n("presetTomorrow", "Tomorrow at 9 AM"),
            time: tomorrow,
        });
    ...
    function createPresetsContainer(presets, confirmBtn, customSection, state) {
        ...
        presets.forEach((preset) => {
            ...
            presetBtn.addEventListener("click", () => {
                presetsContainer
                    .querySelectorAll(".wa-reminder-preset")
                    .forEach((el) => {
                ...
                confirmBtn.textContent = i18n(
                    "confirmReminder",
                    [formatPromptTime(preset.time)],
                    `Set reminder for ${formatPromptTime(preset.time)}`,
                );
    ...
    function createCustomDateTimeSection(confirmBtn, state) {
        ...
        const updateCustomTime = () => {
            ...
            confirmBtn.textContent = i18n(
                "confirmReminder",
                [formatPromptTime(customDate)],
                `Set reminder for ${formatPromptTime(customDate)}`,
            );
            confirmBtn.disabled = false;
        };
    ...
    function createPromptUI(context) {
        ...
        const { section: customSection } = createCustomDateTimeSection(
            confirmBtn,
            state,
        );

        // Build preset buttons
        const presetsContainer = createPresetsContainer(
            presets,
            confirmBtn,
            customSection,
            state,
    ...
    function showSuccess(overlay, context, scheduledTime) {
        ...
        detail.textContent = i18n(
            "reminderCreatedDetail",
            [context.chatName, formatPromptTime(new Date(scheduledTime))],
            `You'll be reminded to follow up with ${context.chatName} at ${formatPromptTime(new Date(scheduledTime))}`,
        );
        ...
        confirmBtn.addEventListener("click", async () => {
            ...
            try {
                ...
                if (response && response.success) {
                    ...
                    const errorMsg =
                        (response && response.error) ||
                        i18n("storageError", "Failed to create reminder");
